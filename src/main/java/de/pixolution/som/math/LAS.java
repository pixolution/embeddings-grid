 ...
 */
public class LAS implements SorterInterface {
	...
	protected final boolean wrappedMode;
    
    protected float[][] distLutF;
    protected int[][] distLut;
    
	public LAS(DistanceFunction<float[]> distanceFunction, Random random, boolean wrappedMode) {
		this.random = random;
	...
	public Grid arrange(Grid imageGrid) {		
		...
		final EmbeddingData[] swapedElements = Arrays.copyOf(imageGrid.getElements(), gridSize);
		...
		do {				
			...
			for (int i = 0; i < NumFilters; i++) 
				map = filterMap(radiusX, radiusY, columns, rows, map, dim, wrappedMode);
	...
	private void copyFeatureVectorsToMap(Grid imageGrid, float[][] map, float alpha) {
	...
	private void checkAllSwaps(Grid imageGrid, float[][] map, EmbeddingData[] swapedElements) {
