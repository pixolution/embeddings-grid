 ...
 */
public class FLAS extends LAS {
	
	protected static final float WeightHole = 0.01f;
	protected static final float WeightSwappable = 1f;
	protected static final float WeightNonSwappable = 10f;
	// quantized distance steps
	protected static final int   QUANT = 256;  
	
	// hyper parameter 
	public static int   MaxSwapPositions = 9;
	// 1 the fraction of swaps per iteration
	public static float SampleFactor = 1.0f;
	static {
		FLAS.InitialRadiusFactor = 0.5f;
		// for small sets this value should be increased ( < 1 ! )
	...
	// fixed map positions
    private boolean[] fixedElements = null; 
    
    // temporary variables
    private int[] swapPositions;
    private EmbeddingData[] swapedElements;
    private float[][] fvs;
    private float[][] mapFvs;

    /**
     * Calculate the image grid by using the algorithm from the paper "Improved Evaluation and Generation 
     * of Grid Layouts using Distance Preservation Quality and Linear Assignment Sorting" (https://arxiv.org/abs/2205.04255v2)
     * @param distanceFunction The distance function used for evaluate the progress
     * @param random The random function to use.
     * @param wrappedMode
     */
	public FLAS(DistanceFunction<float[]> distanceFunction, Random random, boolean wrappedMode) {
		super(distanceFunction, random, wrappedMode);
	}
	
	@Override
	public Grid arrange(Grid imageGrid) {
		// mark all fixed elements
		EmbeddingData[] elements = imageGrid.getElements();
		this.fixedElements = new boolean[elements.length];
		// generate a mask which elements are fixed. Holes are also seen as fixed
		for (int a=0;a<elements.length;a+=1) {
			this.fixedElements[a] = elements[a] != null ? elements[a].isSeeded(): false;
		}
		...
		do {
			...
			for (int i = 0; i < NumFilters; i++) 
				map = filterWeightedMap(radiusX, radiusY, columns, rows, map, dim, weights, wrappedMode);
	...
	private void copyFeatureVectorsToMap(Grid imageGrid, float[][] map, float[] weights, int dim) {
		final EmbeddingData[] elements = imageGrid.getElements();
	...
	protected static float[] filterHwrap(float[] input, int rows, int columns, int filterSize) {
		...
		// size of the border extension
		int ext = filterSize/2;
		// extended row
		float[] rowExt = new float[columns + 2*ext];
		// filter the rows
	...
	private void checkRandomSwaps(int radius, Grid imageGrid, float[][] map) {
		final int columns = imageGrid.getColumns();
		final int rows = imageGrid.getRows();
	...
	private void doSwaps(int[] swapPositions, int numSwapPositions, Grid imageGrid, float[][] map) { 
		
		int numValid = 0;
		for (int i = 0; i < numSwapPositions; i++) {
			final int swapPosition = swapPositions[i];
			final EmbeddingData swapedElement = swapedElements[i] = imageGrid.getElement(swapPosition);
